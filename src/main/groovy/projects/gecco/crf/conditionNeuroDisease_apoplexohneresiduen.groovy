...
condition {
  ...
  final def crfItemNeuro = context.source[studyVisitItem().crf().items()].find {
    "COV_GECCO_NEURO_ERKRANKUNG_ZN_APOPLEX_OHNE_RESIDUEN" == it[CrfItem.TEMPLATE]?.getAt(CrfTemplateField.LABOR_VALUE)?.getAt(LaborValue.CODE)
  }

  if (crfItemNeuro[CrfItem.CATALOG_ENTRY_VALUE] != []) {
    id = "Condition/NeuroDisease-ApoplexOhneResiduen-" + context.source[studyVisitItem().crf().id()]
    ...
    code {
      ...
      final def SNOMEDcode = "429993008"
      if (SNOMEDcode != "") {
        coding {
          system = "http://snomed.info/sct"
          code = SNOMEDcode
