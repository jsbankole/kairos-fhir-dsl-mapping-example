...
condition {
  ...
  final def crfItemSymptom = context.source[studyVisitItem().crf().items()].find {
    "COV_GECCO_Fieber (GECCO)" == it[CrfItem.TEMPLATE]?.getAt(CrfTemplateField.LABOR_VALUE)?.getAt(LaborValue.CODE)
  }
  if (!crfItemSymptom) {
    return //no export
  }
  ...
  crfItemSymptom[CrfItem.CATALOG_ENTRY_VALUE]?.each { final item ->
    ...
    if (SNOMEDcode == "COV_JA") {
      final def crfItemSeverity = context.source[studyVisitItem().crf().items()].find {
        "COV_GECCO_Fieber (GECCO)_SCHWEREGRAD" == it[CrfItem.TEMPLATE]?.getAt(CrfTemplateField.LABOR_VALUE)?.getAt(LaborValue.CODE)
      }
