...
condition {
  ...
  final def crfItemCardiovascular = context.source[studyVisitItem().crf().items()].find {
    "COV_GECCO_HERZKREISLAUF_HERZINSUFFIZIENZ" == it[CrfItem.TEMPLATE]?.getAt(CrfTemplateField.LABOR_VALUE)?.getAt(LaborValue.CODE)
  }

  if (crfItemCardiovascular[CrfItem.CATALOG_ENTRY_VALUE] != []) {
    id = "Condition/ChronicCardiovascularDisease-Herzinsuffizienz-" + context.source[studyVisitItem().crf().id()]
