...
condition {
  ...
  final def crfItemNeuro = context.source[studyVisitItem().crf().items()].find {
    "COV_GECCO_NEURO_ERKRANKUNG_CHRONISCHE_NEUROLOGISCHE_ERKRANKUNG" == it[CrfItem.TEMPLATE]?.getAt(CrfTemplateField.LABOR_VALUE)?.getAt(LaborValue.CODE)
  }

  if (crfItemNeuro[CrfItem.CATALOG_ENTRY_VALUE] != []) {
    id = "Condition/NeuroDisease-ChronischeNeurologischeErkrankung-" + context.source[studyVisitItem().crf().id()]
    ...
    code {
      final def ICDcode = "G96.9"
      if (ICDcode != "") {
        coding {
          system = "http://fhir.de/CodeSystem/dimdi/icd-10-gm"
          version = "2020"
          code = ICDcode
          display = "Chronic nervous system disorder"
        }
      }

      final def SNOMEDcode = "128283000"
      if (SNOMEDcode != "") {
        coding {
          system = "http://snomed.info/sct"
          code = SNOMEDcode
