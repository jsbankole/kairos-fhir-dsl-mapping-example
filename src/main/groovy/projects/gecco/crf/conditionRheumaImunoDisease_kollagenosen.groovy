...
condition {
  ...
  final def crfItemRheum = context.source[studyVisitItem().crf().items()].find {
    "COV_GECCO_IMMONOLOGISCHE_ERKRANKUNG_KOLLAGENOSEN" == it[CrfItem.TEMPLATE]?.getAt(CrfTemplateField.LABOR_VALUE)?.getAt(LaborValue.CODE)
  }

  if (crfItemRheum[CrfItem.CATALOG_ENTRY_VALUE] != []) {
    id = "Condition/ChronicLungDisease-Kollagenosen-" + context.source[studyVisitItem().crf().id()]
    ...
    code {
      ...
      final def SNOMEDcode = "105969002"
      if (SNOMEDcode != "") {
        coding {
          system = "http://snomed.info/sct"
          code = SNOMEDcode
