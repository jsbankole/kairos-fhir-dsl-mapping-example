...
condition {
  ...
  final def crfItemRheum = context.source[studyVisitItem().crf().items()].find {
    "COV_GECCO_IMMONOLOGISCHE_ERKRANKUNG_CHRONISCH_ENTZUENDLICHE_DARMERKRANKUNG" == it[CrfItem.TEMPLATE]?.getAt(CrfTemplateField.LABOR_VALUE)?.getAt(LaborValue.CODE)
  }

  if (crfItemRheum[CrfItem.CATALOG_ENTRY_VALUE] != []) {
    id = "Condition/ChronicLungDisease-ChronischEntzuendlicheDarmerkrankung-" + context.source[studyVisitItem().crf().id()]
