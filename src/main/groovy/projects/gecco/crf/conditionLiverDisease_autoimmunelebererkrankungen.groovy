...
condition {
  ...
  final def crfItemLiver = context.source[studyVisitItem().crf().items()].find {
    "COV_GECCO_LEBERERKRANKUNG_AUTOIMMUNE_LEBERERKRANKUNGEN" == it[CrfItem.TEMPLATE]?.getAt(CrfTemplateField.LABOR_VALUE)?.getAt(LaborValue.CODE)
  }

  if (crfItemLiver[CrfItem.CATALOG_ENTRY_VALUE] != []) {
    id = "Condition/ChronicLiverDisease-AutoimmuneLebererkrankungen-" + context.source[studyVisitItem().crf().id()]
    ...
    code {
      ...
      if (ICDcode != "") {
        coding {
          ...
          version = "2020"
          code = ICDcode
          display = "Autoimmune liver disease"
        }
      }

      final def SNOMEDcode = "235890007"
      if (SNOMEDcode != "") {
        coding {
          system = "http://snomed.info/sct"
          code = SNOMEDcode
