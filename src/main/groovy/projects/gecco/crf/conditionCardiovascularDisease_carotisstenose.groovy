...
condition {
  ...
  final def crfItemCardiovascular = context.source[studyVisitItem().crf().items()].find {
    "COV_GECCO_HERZKREISLAUF_CAROTISSTENOSE" == it[CrfItem.TEMPLATE]?.getAt(CrfTemplateField.LABOR_VALUE)?.getAt(LaborValue.CODE)
  }

  if (crfItemCardiovascular[CrfItem.CATALOG_ENTRY_VALUE] != []) {
    id = "Condition/ChronicCardiovascularDisease-Carotisstenose-" + context.source[studyVisitItem().crf().id()]
