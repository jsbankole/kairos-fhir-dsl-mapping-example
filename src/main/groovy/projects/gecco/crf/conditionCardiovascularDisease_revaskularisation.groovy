...
condition {
  ...
  final def crfItemCardiovascular = context.source[studyVisitItem().crf().items()].find {
    "COV_GECCO_HERZKREISLAUF_REVASKULARISATION" == it[CrfItem.TEMPLATE]?.getAt(CrfTemplateField.LABOR_VALUE)?.getAt(LaborValue.CODE)
  }

  if (crfItemCardiovascular[CrfItem.CATALOG_ENTRY_VALUE] != []) {
    id = "Condition/ChronicCardiovascularDisease-Revaskularisation-" + context.source[studyVisitItem().crf().id()]
    ...
    code {
      final def ICDcode = "Z95.1"
      if (ICDcode != "") {
        coding {
          system = "http://fhir.de/CodeSystem/dimdi/icd-10-gm"
          version = "2020"
          code = ICDcode
          display = "History of coronary artery bypass grafting"
        }
      }

      final def SNOMEDcode = "399261000"
      if (SNOMEDcode != "") {
        coding {
          system = "http://snomed.info/sct"
          code = SNOMEDcode
