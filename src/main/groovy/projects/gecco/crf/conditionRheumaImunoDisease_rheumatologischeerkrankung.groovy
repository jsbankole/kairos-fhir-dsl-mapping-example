...
condition {
  ...
  final def crfItemRheum = context.source[studyVisitItem().crf().items()].find {
    "COV_GECCO_IMMONOLOGISCHE_ERKRANKUNG_RHEUMATOLOGISCHE_ERKRANKUNG" == it[CrfItem.TEMPLATE]?.getAt(CrfTemplateField.LABOR_VALUE)?.getAt(LaborValue.CODE)
  }

  if (crfItemRheum[CrfItem.CATALOG_ENTRY_VALUE] != []) {
    id = "Condition/ChronicLungDisease-RheumatologischeErkrankung-" + context.source[studyVisitItem().crf().id()]
    ...
    code {
      ...
      final def SNOMEDcode = "396332003"
      if (SNOMEDcode != "") {
        coding {
          system = "http://snomed.info/sct"
          code = SNOMEDcode
