...
condition {
  ...
  final def crfItemCardiovascular = context.source[studyVisitItem().crf().items()].find {
    "COV_GECCO_HERZKREISLAUF_BLUTHOCHDRUCK" == it[CrfItem.TEMPLATE]?.getAt(CrfTemplateField.LABOR_VALUE)?.getAt(LaborValue.CODE)
  }

  if (crfItemCardiovascular[CrfItem.CATALOG_ENTRY_VALUE] != []) {
    id = "Condition/ChronicCardiovascularDisease-Bluthochdruck-" + context.source[studyVisitItem().crf().id()]
