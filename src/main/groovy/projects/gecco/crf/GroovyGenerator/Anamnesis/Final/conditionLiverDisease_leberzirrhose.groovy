...
condition {
  ...
  final def crfItemLiver = context.source[studyVisitItem().crf().items()].find {
    "COV_GECCO_LEBERERKRANKUNG_LEBERZIRRHOSE" == it[CrfItem.TEMPLATE]?.getAt(CrfTemplateField.LABOR_VALUE)?.getAt(LaborValue.CODE)
  }

  if (crfItemLiver[CrfItem.CATALOG_ENTRY_VALUE] != []) {
    id = "Condition/ChronicLiverDisease-Leberzirrhose-" + context.source[studyVisitItem().crf().id()]
