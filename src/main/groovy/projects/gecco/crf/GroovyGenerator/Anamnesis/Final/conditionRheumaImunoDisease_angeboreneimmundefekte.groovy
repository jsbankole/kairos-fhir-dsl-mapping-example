...
condition {
  ...
  final def crfItemRheum = context.source[studyVisitItem().crf().items()].find {
    "COV_GECCO_IMMONOLOGISCHE_ERKRANKUNG_ANGEBORENE_IMMUNDEFEKTE" == it[CrfItem.TEMPLATE]?.getAt(CrfTemplateField.LABOR_VALUE)?.getAt(LaborValue.CODE)
  }

  if (crfItemRheum[CrfItem.CATALOG_ENTRY_VALUE] != []) {
    id = "Condition/ChronicLungDisease-AngeboreneImmundefekte-" + context.source[studyVisitItem().crf().id()]
    ...
    crfItemRheum[CrfItem.CATALOG_ENTRY_VALUE]?.each { final item ->
    ...
    code {
      ...
      if (ICDcode != "") {
        coding {
          ...
          version = "2020"
          code = ICDcode
          display = "Congenital immunodeficiency disease"
        }
      }

      final def SNOMEDcode = "36138009"
      if (SNOMEDcode != "") {
        coding {
          system = "http://snomed.info/sct"
          code = SNOMEDcode
    ...
    recordedDate {
      date = normalizeDate(crfItemRheum[CrfItem.CREATIONDATE] as String)
