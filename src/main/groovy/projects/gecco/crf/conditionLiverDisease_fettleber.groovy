...
condition {
  ...
  final def crfItemLiver = context.source[studyVisitItem().crf().items()].find {
    "COV_GECCO_LEBERERKRANKUNG_FETTLEBER" == it[CrfItem.TEMPLATE]?.getAt(CrfTemplateField.LABOR_VALUE)?.getAt(LaborValue.CODE)
  }

  if (crfItemLiver[CrfItem.CATALOG_ENTRY_VALUE] != []) {
    id = "Condition/ChronicLiverDisease-Fettleber-" + context.source[studyVisitItem().crf().id()]
    ...
    code {
      ...
      final def SNOMEDcode = "197321007"
      if (SNOMEDcode != "") {
        coding {
          system = "http://snomed.info/sct"
          code = SNOMEDcode
